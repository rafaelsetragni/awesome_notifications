...
Future<void> delayNotification(int id) async {
  ...
  await AwesomeNotifications().createNotification(
      ...
      schedule: NotificationSchedule(
          initialDateTime: fiveSecondsDelayed
      )
  );
}
...
Future<void> redNotification(int id, bool delayLEDTests) async {
  ...
  await AwesomeNotifications().createNotification(
      ...
      schedule: delayLEDTests ? NotificationSchedule(
...
Future<void> showNotificationWithNoSound(int id) async {
  ...
  await AwesomeNotifications().createNotification(
      content: NotificationContent(
          ...
          body: 'Shhhhhh!!!',
          notificationLayout: NotificationLayout.BigPicture,
          bigPicture: 'https://image.freepik.com/fotos-gratis/medico-serio-mostrando-o-gesto-de-silencio_1262-17188.jpg',
...
Future<void> showBigPictureNetworkNotification(int id) async {
  ...
  await AwesomeNotifications().createNotification(
      content: NotificationContent(
          ...
          body: '$lorenIpsumText\n\n$lorenIpsumText\n\n$lorenIpsumText',
          bigPicture: 'https://images.freeimages.com/images/large-previews/d32/space-halo-2-1626962.jpg',
          notificationLayout: NotificationLayout.BigPicture,
          payload: {
            'uuid' : 'uuid-test'
          }
...
Future<void> showLargeIconNotification(int id) async {
  ...
  await AwesomeNotifications().createNotification(
      content: NotificationContent(
          ...
          body: '$lorenIpsumText\n\n$lorenIpsumText\n\n$lorenIpsumText',
          largeIcon: 'https://image.freepik.com/vetores-gratis/modelo-de-logotipo-de-restaurante-retro_23-2148451519.jpg',
          notificationLayout: NotificationLayout.BigPicture,
          payload: {
            'uuid' : 'uuid-test'
          }
...
void updateNotificationMediaPlayer(int id, MediaModel mediaNow) {
  ...
  AwesomeNotifications().createNotification(
      ...
      actionButtons: [
        NotificationActionButton(
            key: 'MEDIA_PREV',
            icon: 'resource://drawable/ic_prev' + (MediaPlayerCentral.hasPreviousMedia ? '' : '_disabled'),
            label: 'Previous',
            autoCancel: false,
        ...
        MediaPlayerCentral.isPlaying ?
          ...
          NotificationActionButton(
              key: 'MEDIA_PLAY',
              icon: 'resource://drawable/ic_play' + (MediaPlayerCentral.hasAnyMedia ? '' : '_disabled'),
              label: 'Play',
              autoCancel: false,
        ...
        NotificationActionButton(
            key: 'MEDIA_NEXT',
            icon: 'resource://drawable/ic_next' + (MediaPlayerCentral.hasNextMedia ? '' : '_disabled'),
            label: 'Previous',
...
Future<void> listScheduledNotifications(BuildContext context) async {
  ...
  List<PushNotification> activeSchedules =  await AwesomeNotifications().listScheduledNotifications();
  ...
  return showDialog<void>(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        content: Text(
            '${activeSchedules.length} schedules founded'),
        actions: [
          FlatButton(
            child: Text('OK'),
            onPressed: () {
...
Future<void> showNotificationAtScheduleCron(int id, DateTime scheduleTime) async {
  ...
  await AwesomeNotifications().createNotification(
      content: NotificationContent(
          ...
          channelKey: 'schedule',
          title: 'Just in time!',
          body: 'This notification was schedule to shows at '+DateUtils.parseDateToString(scheduleTime.toLocal()),
          notificationLayout: NotificationLayout.BigPicture,
  ...
  await AwesomeNotifications().createNotification(
      ...
      schedule: NotificationSchedule(
          crontabSchedule: CronHelper.instance.workweekDay(
              referenceUtcDate: DateUtils.parseStringToDate('10:00',format: 'HH:mm').toUtc()
          )
      )
  );
}
