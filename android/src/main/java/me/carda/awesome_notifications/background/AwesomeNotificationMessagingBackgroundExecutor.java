 ...
 */
public class AwesomeNotificationMessagingBackgroundExecutor implements MethodCallHandler {
  static final String SHARED_PREFERENCES_KEY = "awesome.notification.messaging.callback";

  private static final String TAG = "AWNBGExecutor";
  ...
  private void onInitialized() {
    ...
    AwesomeNotificationMessagingBackgroundService.onInitialized();
  }
  ...
  public void onMethodCall(MethodCall call, @NonNull Result result) {
    ...
    try {
      if (method.equals(Definitions.CHANNEL_METHOD_INIT_BACKGROUND)) {
      ...
      result.error("error", "Flutter Awesome notification error: " + e.getMessage(), null);
    }
  }
  ...
  public void startBackgroundIsolate(long callbackHandle, FlutterShellArgs shellArgs) {
    ...
    Runnable myRunnable =
        () -> {
          FlutterInjector.instance().flutterLoader().startInitialization(ContextHolder.getApplicationContext());
  ...
  public void executeDartCallbackInBackgroundIsolate(Intent intent, final CountDownLatch latch) {
    ...
    Serializable actionReceived = intent.getSerializableExtra("action");
    if (actionReceived != null) {
      Map<String, Object> actionReceivedMap = new HashMap<>();
      actionReceivedMap.put(Definitions.ARGUMENT_BACKGROUND_ACTION_RECEIVED,actionReceived);
      backgroundChannel.invokeMethod(
          Definitions.CHANNEL_METHOD_ACTION_RECEIVED_BACKGROUND,
          new HashMap<String, Object>() {
            {
              put("userCallbackHandle", getUserCallbackHandle());
              put("message", actionReceivedMap);
            }
          },
          result);
    } else {
  ...
  private void initializeMethodChannel(BinaryMessenger isolate) {
    ...
    // - "CHANNEL_FLUTTER_BACKGROUND_PLUGIN"
    //
    ...
    backgroundChannel =
        new MethodChannel(isolate, Definitions.CHANNEL_FLUTTER_BACKGROUND_PLUGIN);
